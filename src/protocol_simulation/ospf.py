...
class OSPFAsynchronousWorkspaceDashboard:
    ...
    def render_base_configuration_graph(self):
        ...
        self.ax_f.set_title(f"OSPF Network Topology — Configuration Mode  [{topo_name}]", fontsize=12, fontweight='bold', color="#2c3e50")
        self.ax_f.axis('off')
        self.canvas_f.draw()
    ...
    def update_text_panels_data(self):
        ...
        if self.selected_edge is None:
            ...
            self.cost_change_combo.set(str(state["get_cost_func"](u, v, current_clock_time)))
    ...
    def render_flooding_state_view(self):
        ...
        topo_name = self.topology_combo.get() if hasattr(self, 'topology_combo') else ''
        self.ax_f.set_title(f"Asynchronous OSPF Flooding Clock: {format_time(current_clock_time)}  [{topo_name}]", fontsize=12, fontweight='bold', color="#2c3e50")
        self.ax_f.axis('off')
        self.canvas_f.draw()
